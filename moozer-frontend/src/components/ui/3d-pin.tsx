...
export const PinContainer = ({
	...
	return (
		<a
			// You could remove the fixed sm:w-10 h-10 and go fully fluid if you like:
			className={cn('relative group/pin z-50 cursor-pointer sm:w-10 h-10', containerClassName)}
		...
		>
            ...
            rounded-2xl shadow-[0_8px_16px_rgb(0_0_0/0.4)] bg-base-200 
...
export const PinPerspective = ({ title, href }: { title?: string; href?: string }) => {
	return (
		<motion.div
			// Instead of w-96 h-80, we use w-full + max-w for responsiveness:
			className='
        pointer-events-none 
        w-full max-w-sm 
		...
		>
			<div className='relative w-full h-full'>
              ...
              ring-1 ring-white/10'
					>
						<span className='relative z-20 text-white text-xs font-bold py-0.5'>{title}</span>
						<span
				...
				{/* Glowing circles + perspective container */}
				<div
					style={{
						perspective: '1000px',
              ...
              '
				...
				{/* Extra visuals (pin tail, dot, etc.) */}
				<>
					<motion.div
						className='
