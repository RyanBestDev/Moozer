...
var multiSession = (options) => {
  ...
  return {
    id: "multi-session",
    endpoints: {
      listDeviceSessions: chunkFTP5NPIZ_cjs.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const cookies = Object.fromEntries(chunkJOH6QBAP_cjs.parseCookies(cookieHeader));
      ...
      setActiveSession: chunkFTP5NPIZ_cjs.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          await chunkJOH6QBAP_cjs.setSessionCookie(ctx, session);
          return ctx.json(session);
        }
      ),
      revokeDeviceSession: chunkFTP5NPIZ_cjs.createAuthEndpoint(
          ...
          if (cookieHeader) {
            ...
            if (sessionTokens.length > 0) {
              ...
              if (validSessions.length > 0) {
                ...
                await chunkJOH6QBAP_cjs.setSessionCookie(ctx, nextSession);
              } else {
                chunkJOH6QBAP_cjs.deleteSessionCookie(ctx);
              }
            } else {
              chunkJOH6QBAP_cjs.deleteSessionCookie(ctx);
            }
      ...
      after: [
        {
          ...
          handler: chunkFTP5NPIZ_cjs.createAuthMiddleware(async (ctx) => {
            const headers = ctx.context.returned instanceof chunkFTP5NPIZ_cjs.APIError ? ctx.context.returned.headers : ctx.responseHeader;
            const cookieString = headers.get("set-cookie");
            if (!cookieString) return;
            const setCookies = chunkJOH6QBAP_cjs.parseSetCookieHeader(cookieString);
            const sessionCookieConfig = ctx.context.authCookies.sessionToken;
            const sessionToken = setCookies.get(
              sessionCookieConfig.name
            )?.value;
            if (!sessionToken) return;
            const cookies = chunkJOH6QBAP_cjs.parseCookies(ctx.headers?.get("cookie") || "");
