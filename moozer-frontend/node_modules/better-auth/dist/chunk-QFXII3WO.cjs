...
async function generator(ctx, options) {
  const baseEndpoints = chunkFTP5NPIZ_cjs.getEndpoints(ctx, {
    ...options,
    plugins: []
  });
  const tables = chunkBCGZXLBF_cjs.getAuthTables(options);
  ...
  for (const plugin of options.plugins || []) {
    ...
    const pluginEndpoints = chunkFTP5NPIZ_cjs.getEndpoints(ctx, {
      ...options,
      plugins: [plugin]
    });
...
var openAPI = (options) => {
  ...
  return {
    id: "open-api",
    endpoints: {
      generateOpenAPISchema: chunkFTP5NPIZ_cjs.createAuthEndpoint(
      ...
      openAPIReference: chunkFTP5NPIZ_cjs.createAuthEndpoint(
        path,
        {
          method: "GET",
          metadata: {
