...
var chunkNU4SXYJ5_cjs = require('./chunk-NU4SXYJ5.cjs');
...
var getSession = () => createAuthEndpoint(
  ...
  async (ctx) => {
    try {
      ...
      const sessionDataPayload = sessionDataCookie ? chunkOA7Z7IZT_cjs.safeJSONParse(binary.binary.decode(base64.base64.decode(sessionDataCookie))) : null;
      ...
      await chunkJOH6QBAP_cjs.setCookieCache(ctx, session);
      return ctx.json(
        session
      );
    } catch (error2) {
...
var signInSocial = createAuthEndpoint(
  "/sign-in/social",
  {
    ...
    body: zod.z.object({
       ...
       */
      provider: chunkNU4SXYJ5_cjs.SocialProviderListEnum,
      /**
  ...
  async (c) => {
    ...
    if (c.body.idToken) {
      ...
      await chunkJOH6QBAP_cjs.setSessionCookie(c, data.data);
      return c.json({
        redirect: false,
...
var callbackOAuth = createAuthEndpoint(
  ...
  async (c) => {
    ...
    await chunkJOH6QBAP_cjs.setSessionCookie(c, {
      session,
      user
    });
    let toRedirectTo;
...
var updateUser = () => createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const additionalFields = chunkUQC2FUXC_cjs.parseUserInput(
      ctx.context.options,
      rest,
      "update"
    );
...
var signUpEmail = () => createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const additionalData = chunkUQC2FUXC_cjs.parseUserInput(
      ctx.context.options,
      additionalFields
    );
    let createdUser;
...
var linkSocialAccount = createAuthEndpoint(
  "/link-social",
  {
    ...
    body: zod.z.object({
       ...
       * OAuth2 provider to use`
       */
      provider: zod.z.enum(chunkNU4SXYJ5_cjs.socialProviderList, {
...
async function onRequestRateLimit(req, ctx) {
  ...
  const key = chunkUQC2FUXC_cjs.getIp(req, ctx.options) + path;
